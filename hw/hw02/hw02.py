...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(index, res, gap):
        if n == 1:
            return 1
        elif index != n:
            if num_eights(index) or index % 8 == 0:
                return helper(index+1, res-gap, -gap)
            else:
                return helper(index+1, res+gap, gap)
        else:
            return res
...
def missing_digits(n):
    ...
    if(n // 10 == 0):
        return 0
    all_but_last, last_one, last_two = n // 10, n % 10, n % 100 // 10
    if all_but_last:
        if(last_one == last_two):
            return missing_digits(all_but_last)
        else:
            return missing_digits(all_but_last) + last_one - last_two - 1
